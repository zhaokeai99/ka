...
import { CouponAfterContext } from '../data.d';
...
const BondBaseInfoCard = () => {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data } = await getBondBaseInfo({ bondCode: bondCode });
      setBaseInfo({
        ...data,
      });
      setLoading(false);
  ...
  const onNavigateClick = (code: any, name: any) => () => {
    history.push(`/information/priceChanges/subjectDetail/${name}/${code}`);
  };
