...
export enum CouponAfterTabsKeys {
  VALUATION = 'valuation',
  CLOSE = 'close',
  RELATIVE = 'relative',
}
...
export interface ICouponAfterContext {
...
export const initCAcontextValue: ICouponAfterContext = {
  title: '',
  text: '',
  visible: false,
  valueEnum: {},
...
export interface ICommonStateType extends Omit<ICouponAfterContext, 'onOpen'> {}

export interface ICloseEventModalParams extends Omit<ICouponAfterContext, 'valueEnum' | 'onOpen'> {}

export interface ICommonActionType {
  payload?: ICommonStateType;
  type: 'OPEN' | 'CLOSE' | 'EVENT-ENUM';
}

export const CouponAfterContext = React.createContext<ICouponAfterContext>(initCAcontextValue);
...
export interface ITableItemType {
  ...
  eventType: string; //事件归因分类
  eventTitle: string; //事件标题
  eventDetail: string; //事件细节
}
