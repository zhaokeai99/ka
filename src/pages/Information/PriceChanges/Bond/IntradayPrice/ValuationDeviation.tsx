...
import { IValuationTableItem, getDayValOffset, likeBondName, likeCompanyName } from './service';
import moment from 'moment';
import cls from 'classnames';
import styles from '../index.less';

interface IValuationDeviation {
  title: string;
 ...
 */
//TODO
const ValuationDeviation: React.FC<IValuationDeviation> = () => {
  ...
  const reloadDataSource = useCallback(async (params, sort) => {
    const requestParams: Partial<IValuationTableItem & { current: number }> = {};
    ...
    const { data } = await getDayValOffset(requestParams);
  ...
  useEffect(() => {
    ...
    if (bondTypeInitValue?.length <= 0) {
      return;
    }
    (async () => {
      await formRef.current?.setFieldsValue({
        bondType: bondTypeInitValue,
      });
      await formRef.current?.submit();
    })();
  }, [bondType]);

  const searchColumns: ProColumns<IValuationTableItem>[] = [
    {
      title: '日期',
      align: 'center',
      dataIndex: 'dateTime',
  ...
  const columns: ProColumns<IValuationTableItem>[] = [
    {
      title: '时间',
      align: 'left',
      dataIndex: 'dateTime',
