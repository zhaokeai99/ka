...
import { IYesterdayTableItem, getDayPreValOffset, likeBondName, likeCompanyName } from './service';
import styles from '../index.less';

interface IYesterdayClosedOff {
  title: string;
  value: intradayTabsKeys.PRE;
}

//默认时间
const dateTimeInitValue = [moment().startOf('day'), moment().endOf('day')];

/**
 * 昨收偏离
 */
const YesterdayClosedOff: React.FC<IYesterdayClosedOff> = () => {
  ...
  const onNavigateClick = (bondCode: any) => () => {
    history.push(`/information/priceChanges/couponIntraday/${bondCode}`);
  };

  // 解构请求参数
  const reloadDataSource = useCallback(async (params, sort) => {
    const requestParams: Partial<IYesterdayTableItem & { current: number }> = {};
    Object.keys(params).forEach((item) => {
      switch (item) {
        case 'amount': // 成交金额
          ...
          break;
        case 'dateTime': // 时间
          requestParams['startTime'] = moment(params?.dateTime?.[0]).format('HH:ss:mm');
          requestParams['endTime'] = moment(params?.dateTime?.[1]).format('HH:ss:mm');
          break;
    ...
    const { data } = await getDayPreValOffset(requestParams);
  ...
  const searchColumns: ProColumns<IYesterdayTableItem>[] = [
    {
      title: '日期',
      align: 'center',
      dataIndex: 'dateTime',
      hideInTable: true,
      initialValue: dateTimeInitValue,
      renderFormItem: () => {
        return (
  ...
  const columns: ProColumns<IYesterdayTableItem>[] = [
    {
      title: '时间',
      align: 'left',
      dataIndex: 'dateTime',
    ...
    },
    {
      title: (
        <>
          成交量（手）
          <Tooltip placement="top" title={volumeTootipTitle}>
    ...
    },
    {
      title: (
        <>
          成交金额（万元）
          <Tooltip placement="top" title={amountTootipTitle}>
            <QuestionCircleOutlined style={{ marginInlineStart: 4 }} />
          </Tooltip>
        </>
      ),
...
export default YesterdayClosedOff;
