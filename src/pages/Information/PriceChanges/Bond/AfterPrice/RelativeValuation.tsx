...
import { IRelativeTableItem, getYestDayValOffset, likeBondName, likeCompanyName } from './service';

import moment from 'moment';
import cls from 'classnames';
import styles from '../index.less';

interface IRelativeValuation {
  title: string;
  value: afterTabsKeys.RELATIVE;
}
 ...
 * 相对估值偏离
 */
const RelativeValuation: React.FC<IRelativeValuation> = () => {
  ...
  const reloadDataSource = useCallback(async (params, sort) => {
    const requestParams: Partial<IRelativeTableItem & { current: number }> = {};
    ...
    const { data } = await getYestDayValOffset(requestParams);
  ...
  const searchColumns: ProColumns<IRelativeTableItem>[] = [
    {
      title: '日期',
      align: 'center',
      dataIndex: 'dateTime',
  ...
  const columns: ProColumns<IRelativeTableItem>[] = [
    {
      title: '时间',
      align: 'left',
      dataIndex: 'dateTime',
