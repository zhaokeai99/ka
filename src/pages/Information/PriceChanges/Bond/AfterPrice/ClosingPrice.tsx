 ...
 * 收盘价格异动
 */
const ClosingPrice: React.FC<IClosingPrice> = () => {
  ...
  const reloadDataSource = useCallback(async (params, sort) => {
    const requestParams: Partial<IClosingTableItem & { current: number }> = {};
    ...
    const { data } = await getYestDayClosePriceOffset(requestParams);
    return {
      ...data,
    };
  }, []);
  ...
  const searchColumns: ProColumns<IClosingTableItem>[] = [
    {
      title: <span>日期</span>,
      align: 'center',
      dataIndex: 'dateTime',
  ...
  const columns: ProColumns<IClosingTableItem>[] = [
    {
      title: '时间',
      align: 'left',
      dataIndex: 'dateTime',
    ...
    },
    {
      ...
      render: (text) => {
        return (
          <div style={{ textAlign: 'left' }} title={text as string}>
            {text}
          </div>
        );
      },
