    ...
    (async () => {
      ...
      if (result?.length) {
        ...
        if (result[0]?.children?.length) {
          const { nodeName, nodeId } = result[0]?.children[0] || {};
  ...
  return (
    <ProCard size="small" gutter={[0, 8]} style={{ paddingTop: 24, paddingBottom: 0 }}>
      ...
      >
        <Row>
          <Col span={5}>
            <Form.Item label="情绪" name="publicSentiment">
          ...
          <Col span={5}>
            <Form.Item label="节点" name="chainNodeName">
              <Select allowClear placeholder="请选择节点">
                {(nodeData || []).map(({ nodeName, nodeId, children = [] }) => (
                  <OptGroup key={nodeId} label={nodeName}>
                    {children?.map(({ nodeId: val, nodeName: title }) => {
                      return (
                        <Option key={val} value={val}>
                          {title}
                        </Option>
